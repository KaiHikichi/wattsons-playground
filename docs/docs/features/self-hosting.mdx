...
If you also want to self-host these services, you can use the [Docker setup](../advanced/docker.mdx) (for VPS) or the [PHP setup](../advanced/php.mdx) (for shared hosting).
...
The app can be deployed as a [Cloudflare Worker with Static Assets](https://developers.cloudflare.com/workers/static-assets/), which is not the same product as Cloudflare Pages listed under [Git Integration](#git-integration) above.

Create a `wrangler.jsonc` at the repo root:
...
Note: `build` also contains `build/docs/` (this documentation site) alongside `build/livecodes/` (the app) when built with `npm run build`. Deploying the whole folder serves both; if you only want the app, use `npm run build:app` instead of the full `build` script, though this documentation site's docs directory does not conflict with the app and is generally safe to leave in place.

Build with `BASE_URL="/"` so the app is served from the Worker's own root, rather than the GitHub Pages-specific `/livecodes/` subdirectory default:

```shell
npm run deploy:cloudflare
```

Additionally, several GitHub Actions workflows are included to automatically build and deploy the app when a new commit is pushed (e.g. to the `main` branch). Deploy targets include GitHub Pages, FTP and SSH. See the [Deployment guide](../advanced/deployment.mdx) for the available deploy actions, when they are triggered, and the required secrets and variables.

### Docker Setup

In addition to the _static_ app provided by the previous methods, the included [Docker setup](../advanced/docker.mdx) provides alternative implementations for server-side features available in the [hosted app](https://livecodes.io),
e.g. automatic HTTPS, [Open Graph meta tags](https://ogp.me/), [oEmbed](https://oembed.com/), custom headers, [short-URL share](./share.mdx), [broadcast server](./broadcast.mdx), separate origin sandbox to run code, custom 404 page, etc.
...
For shared hosting that supports PHP and MySQL, the included [PHP setup](../advanced/php.mdx) provides the same services as the Docker setup, without installing anything on the server. It is deployed via FTP/SSH using the included GitHub Actions workflow.

## Deployment

For continuous deployment, you can use the included [GitHub Actions](https://github.com/features/actions) workflows to automatically build and deploy the app when a new commit is pushed (e.g. to the `main` branch).

Deploy targets include [GitHub Pages](https://pages.github.com/), FTP, SSH, the [Docker setup](../advanced/docker.mdx) (VPS) and the [PHP setup](../advanced/php.mdx) (shared hosting).

See the [Deployment guide](../advanced/deployment.mdx) for the available deploy actions, when they are triggered, and the required secrets and variables.
...
This is an example of a self-hosted deployment, that was deployed to shared hosting using the included [PHP setup](../advanced/php.mdx):
