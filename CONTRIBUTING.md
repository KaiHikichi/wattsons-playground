...
`functions/vendors/templates.js` is generated by `npm run build`
(`scripts/templates-json.mjs`) from the starter templates. Don't edit it by
hand; after adding or renaming a starter, rebuild and commit the result. The
deployed Worker serves only `build/` (`wrangler.jsonc`), so `functions/` is
not live on play.hellowattson.ca.
